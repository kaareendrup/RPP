...
class RegressionPlotter(Plotter):
        ...
        for model_list in [self._models_list, self._benchmarks_list]:
            for model in model_list:
                ...
                fig, axs = plt.subplots(
                    ...
                    figsize=(18, 7),
                    sharey=True,
                    gridspec_kw={"wspace": 0.04, "width_ratios": [1, 1.2]},
                )
                ...
                h = axs[1].hist2d(
                    model._predictions, model._truths, bins=bins, cmap=self._cmap
                )
                plt.colorbar(h[3], pad=0.01)

                axs[0].set_xlabel(
                    self._target + r"$_\mathrm{true}$" + self._unit_str, size=12
                )
                axs[1].set_xlabel(
                    self._target + r"$_\mathrm{true}$" + self._unit_str, size=12
                )
                axs[0].set_ylabel(
                    self._target + r"$_\mathrm{reco}$" + self._unit_str, size=12
                )

                axs[0].set_title(
                    "Model performance on {}".format(self._target), fontsize=12
                )
                axs[1].set_title(
                    "Model performance on {}".format(self._target), fontsize=12
                )
    ...
    def plot_resolution(
        ...
        self._bin_positions = np.digitize(
            models[0]._truths, self._bin_edges, right=True
        )
        self._bin_centers = (
        ...
        for subplot, relative, label in zip(
            subplots, [False, True], ["absolute" + self._unit_str, "relative"]
        ):
            # Use shorthand
            ax_top, ax_back, ax_bot = subplot[0], subplot[-1], subplot[-2]

            for model, benchmark, compare_color in zip(
                models, benchmarks, self._color_dict["compare"]
            ):
                # Add data
            ...
            ax_bot.set_xlabel(
                self._target + r"$_\mathrm{true}$" + self._unit_str, fontsize=12
            )
