...
class ClassificationSpatialPlotter(ClassificationPlotter):
    ...
    def get_good_bad_pools(
        ...
        for i in range(n_panels):
            ...
            if len(pool) == 0:
                ...
                truths_query = "SELECT event_no, pid, particle_sign FROM truth WHERE event_no == {}".format(
                    event
                )
    ...
    def visualise_discrepancy_plot(
        ...
        for model, benchmark in zip(models, benchmarks):
            ...
            fig, axs = plt.subplots(
                1, n_panels, figsize=(n_panels * 8, 7), subplot_kw=dict(projection="3d")
            )
            ...
            (
                ...
                vmin,
                vmax,
            ) = self.get_good_bad_pools(model, benchmark, colorby, allpanels)

            # Loop over panels
            for ax, event, features, truths, label in zip(
                axs, events, features_list, truths_list, labels
            ):
                # Create label
                label = (
                    r"$\nu_e$"
            ...
            plt.savefig(
                self._plot_dir
                + model._title
                + "_events_3D_single_{}.png".format(colorby)
            )
            plt.close()
    ...
    def plot_event_displays(
        ...
        for model, benchmark in zip(models, benchmarks):
            ...
            # Loop over panels
            for i, (event, features, truths, label, force_rotation) in enumerate(
                zip(events, features_list, truths_list, labels, force_rotate)
            ):
                # Create label
                label = (
                    r"$\nu_e$"
                ...
                # Create subplots
                ax_top = plt.subplot(
                    3, n_panels, (0 * n_panels + i + 1), projection="polar"
                )
                ax_sides = plt.subplot(3, n_panels, (1 * n_panels + i + 1))
                ax_bottom = plt.subplot(
                    3, n_panels, (2 * n_panels + i + 1), projection="polar"
                )
                ...
                features["phi"] = rotate_polar_mean(
                    features["phi"], auto_rotate, force_rotation
                )
            ...
            plt.savefig(
                self._plot_dir
                + model._title
                + "_event_displays_single_{}.png".format(colorby)
            )
            plt.close()
    ...
    def plot_several_event_displays(
        ...
        for model, benchmark in zip(models, benchmarks):
            ...
            for m in range(rows):
                for n in range(columns):
                    (
                        ...
                        _,
                        _,
                    ) = self.get_good_bad_pools(
                        model, benchmark, colorby, False, seeds[m, n]
                    )
                    event, features, truths = (
                        events[0],
                        features_list[0],
                        truths_list[0],
                    )
                    ...
                    ax_top = plt.subplot(
                        3 * rows,
                        columns,
                        (m * 3 * columns + n + 0 * columns + 1),
                        projection="polar",
                    )
                    ax_sides = plt.subplot(
                        3 * rows, columns, (m * 3 * columns + n + 1 * columns + 1)
                    )
                    ax_bottom = plt.subplot(
                        3 * rows,
                        columns,
                        (m * 3 * columns + n + 2 * columns + 1),
                        projection="polar",
                    )
            ...
            plt.savefig(
                self._plot_dir
                + model._title
                + "_event_displays_multiple_{}.png".format(colorby)
            )
            plt.close()
